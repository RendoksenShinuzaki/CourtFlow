...
<body class="bg-gray-200">
    <div>
        <nav class="lg:hidden py-6 px-6 bg-indigo-50">
                ...
                <div class="px-4 pb-6">
                    ...
                    <ul class="text-sm font-medium">
                        ...
                        <li class="<?php echo ($RetrievedDocuments === 0) ? 'hidden' : ''; ?>">
                            <a href="userRetrievedDocuments.php" class="flex items-center pl-3 py-3 pr-2 text-gray-500 hover:bg-gray-700 hover:text-white transition duration-300 border-gray-500 rounded" href="#">
                                <span class="inline-block mr-3">
    ...
    <main class="grid min-h-full place-items-center px-6 py-24 sm:py-32 lg:px-8">
        <div class="text-center flex flex-col items-center">
            <div class="bg-gray-200 bg-opacity-90 rounded-lg p-8 flex flex-col items-center">
                <h1 class="mt-4 text-3xl font-bold tracking-tight text-gray-900 sm:text-5xl">Login Successfully!</h1>
                <?php if (!empty($loggedInLastName) && !empty($loggedInFirstName) && !empty($loggedInMiddleName) && !empty($loggedInGender)) : ?>
                    <p class="mt-6 text-base leading-7 text-gray-600 font-semibold text-lg">Welcome, <?php echo ($loggedInGender === 'male') ? 'Mr.' : 'Ms.'; ?><?php echo " $loggedInLastName, $loggedInFirstName $loggedInMiddleName" ?></p>
                <?php endif; ?>
                <div class="<?php echo ($loggedInRole === 'Fiscal') ? '' : 'hidden'; ?>">
                    <?php
                        $stmt = $conn->prepare("SELECT * FROM v1_pao_submit_case WHERE Date >= DATE_SUB(CURDATE(), INTERVAL 3 DAY) AND Accepted = 0");
                        $stmt->execute();
                        $result = $stmt->get_result();
                        $num = $result->num_rows;
                        
                        if($num > 0){
                            echo "You have ".$num." late cases to be accepted and submitted";
                        }
                    ?>
                </div>
                <div class="<?php echo ($loggedInRole === 'OCC') ? '' : 'hidden'; ?>">
                    <?php
                        $stmt = $conn->prepare("SELECT * FROM v2_fiscal_submit_case WHERE DateSubmitted >= DATE_SUB(CURDATE(), INTERVAL 3 DAY) AND Accepted = 0");
                        $stmt->execute();
                        $result = $stmt->get_result();
                        $num = $result->num_rows;
                ...
                <div class="<?php echo ($loggedInRole === 'RTC') ? '' : 'hidden'; ?>">
                    <?php
                        $stmt = $conn->prepare("SELECT * FROM v3_occ_case_assignment WHERE DateSubmitted >= DATE_SUB(CURDATE(), INTERVAL 3 DAY) AND Accepted = 0");
                        $stmt->execute();
                        $result = $stmt->get_result();
                        $num = $result->num_rows;
                ...
                <div class="<?php echo ($loggedInRole === 'Interpreter') ? '' : 'hidden'; ?>">
                    <?php
                        $stmt = $conn->prepare("SELECT * FROM v4_rtc_submit_warrant WHERE DateSubmitted >= DATE_SUB(CURDATE(), INTERVAL 3 DAY) AND Accepted = 0");
                        $stmt->execute();
                        $result = $stmt->get_result();
                        $num = $result->num_rows;
                ...
                <div class="mt-1 sm:mt-2 md:mt-4"> 
                    <img src="../../images/Welcomee.gif" alt="GIF Image" class="w-60 h-60 md:w-48 md:h-48 lg:w-64 lg:h-64">
                </div>
            </div>
        </div>
    </main>
