...
$PaoSubmitCase = 'v1_pao_submit_case';

// Query for total sent cases
$SentcountQuery = "SELECT COUNT(*) AS total FROM $PaoSubmitCase";
$SentcountResult = $conn->query($SentcountQuery);

// Query for total returned cases
$ReturncountQuery = "SELECT COUNT(*) AS total FROM $PaoSubmitCase WHERE isReturnApproved = '1'";
$ReturncountResult = $conn->query($ReturncountQuery);

// Initialize counts
$PaorowCount = 0;
$Returncount = 0;

// Check and set total sent cases count
if ($SentcountResult) {
    $sentCountData = $SentcountResult->fetch_assoc();
    $PaorowCount = $sentCountData['total'];
} else {
    // Handle error if the query fails
    // You may want to log or display the error message
}

// Check and set total returned cases count
if ($ReturncountResult) {
    $returnCountData = $ReturncountResult->fetch_assoc();
    $Returncount = $returnCountData['total'];
} else {
    // Handle error if the query fails
    // You may want to log or display the error message
}

//===============For Fiscal Dashboard============================
$FiscalSubmitCase = 'v2_fiscal_submit_case';
$FiscalInbox = 'inbox_fiscal';

//Query for Total sent cases
$FiscalSentcountQuery = "SELECT COUNT(*) AS total FROM $FiscalSubmitCase";
$FiscalSentcountResult = $conn->query($FiscalSentcountQuery);

//Query for Total returned cases
$FiscalReturncountQuery = "SELECT COUNT(*) AS total FROM $FiscalSubmitCase  WHERE isReturnApproved = '1'";
$FiscalReturncountResult = $conn->query($FiscalReturncountQuery);

//Query for Total Inbox Cases
$FiscalInboxSql = "SELECT COUNT(*) AS total FROM $FiscalInbox";
$FiscalInboxResult = $conn->query($FiscalInboxSql);

// Initialize counts
$FiscalSentrowCount = 0;
$FiscalReturncount = 0;
$FiscalInboxcount = 0;

// Check and set total sent cases count
if ($FiscalSentcountResult) {
    $sentCountData = $FiscalSentcountResult->fetch_assoc();
    $FiscalSentrowCount = $sentCountData['total'];
} else {
    // Handle error if the query fails
    // You may want to log or display the error message
}
...
// Check and set total returned cases count
if ($FiscalReturncountResult) {
    $returnCountData = $FiscalReturncountResult->fetch_assoc();
    $FiscalReturncount = $returnCountData['total'];
} else {
    // Handle error if the query fails
    // You may want to log or display the error message
}
//Check and set total inbox 
if ($FiscalInboxResult) {
    $returnCountData = $FiscalInboxResult->fetch_assoc();
    $FiscalInboxcount = $returnCountData['total'];
} else {
    // Handle error if the query fails
    // You may want to log or display the error message
}
...
$OCCSubmitCase = 'v3_occ_case_assignment';
$OCCInbox = 'inbox_occ';

//Query for Total sent cases
$OCCSentcountQuery = "SELECT COUNT(*) AS total FROM $OCCSubmitCase";
$OCCSentcountResult = $conn->query($OCCSentcountQuery);

//Query for Total Inbox Cases
$OCCInboxSql = "SELECT COUNT(*) AS total FROM $OCCInbox";
...
// Check and set total sent cases count
if ($OCCSentcountResult) {
    $sentCountData = $OCCSentcountResult->fetch_assoc();
    $OCCSentrowCount = $sentCountData['total'];
} else {
    // Handle error if the query fails
    // You may want to log or display the error message
}
...
//Check and set total inbox 
if ($OCCInboxResult) {
    $returnCountData = $OCCInboxResult->fetch_assoc();
    $OCCInboxcount = $returnCountData['total'];
} else {
    // Handle error if the query fails
    // You may want to log or display the error message
}
...
//===============FOR RTC DASHBOARD=================
$RTCInboxWarrant = 'inbox_rtc';
$RTCInboxSubpoena = 'rtc_for_subpoena';
$RTCSentWarrant = 'v4_rtc_submit_warrant';
$RTCSentSubpoena = 'rtc_submit_subpoena';
$RTCWarrantStatus = 'rtc_warrant';

//Query for Total sent cases
$RTCSentWarrantInboxQuery = "SELECT COUNT(*) AS total FROM $RTCInboxWarrant";
$RTCSentWarrantInboxResult = $conn->query($RTCSentWarrantInboxQuery);

$RTCSentSubpoenaInboxQuery = "SELECT COUNT(*) AS total FROM $RTCInboxSubpoena";
$RTCSentSubpoenaInboxResult = $conn->query($RTCSentSubpoenaInboxQuery);

$RTCSentWarrantcountQuery = "SELECT COUNT(*) AS total FROM $RTCSentWarrant";
$RTCSentWarrantcountResult = $conn->query($RTCSentWarrantcountQuery);

$RTCSentSubpoenaSubmitcountQuery = "SELECT COUNT(*) AS total FROM $RTCSentSubpoena";
$RTCSentSubpoenaSubmitcountResult = $conn->query($RTCSentSubpoenaSubmitcountQuery);

$RTCWarrantPendingSql = "SELECT COUNT(*) AS total FROM $RTCWarrantStatus WHERE isPending = '0'";
$RTCWarrantPendingSqlResult = $conn->query($RTCWarrantPendingSql);

$RTCWarrantStandingSql = "SELECT COUNT(*) AS total FROM $RTCWarrantStatus WHERE isPending = '1'";
$$RTCWarrantStandingSqlResult = $conn->query($RTCWarrantStandingSql);
...
$RTCWarrantCount = 0;
$RTCSubpoenaCount = 0;
$RTCWarrantisPending = 0;
$RTCWarrantisStanding = 0;

// Check and set total sent cases count
if ($RTCSentWarrantInboxResult) {
    $sentCountData = $RTCSentWarrantInboxResult->fetch_assoc();
    $RTCWarrantInbox = $sentCountData['total'];
} else {
    // Handle error if the query fails
    // You may want to log or display the error message
}
...
//Check and set total inbox 
if ($RTCSentSubpoenaInboxResult) {
    $returnCountData = $RTCSentSubpoenaInboxResult->fetch_assoc();
    $RTCSubpoenaInbox = $returnCountData['total'];
} else {
    // Handle error if the query fails
    // You may want to log or display the error message
}

if ($RTCSentWarrantcountResult) {
    $returnCountData = $RTCSentWarrantcountResult->fetch_assoc();
    $RTCWarrantCount = $returnCountData['total'];
} else {
    // Handle error if the query fails
    // You may want to log or display the error message
}

if ($RTCSentSubpoenaSubmitcountResult) {
    $returnCountData = $RTCSentSubpoenaSubmitcountResult->fetch_assoc();
    $RTCSubpoenaCount = $returnCountData['total'];
} else {
    // Handle error if the query fails
    // You may want to log or display the error message
}

if ($RTCWarrantPendingSqlResult) {
    $returnCountData = $RTCWarrantPendingSqlResult->fetch_assoc();
    $RTCWarrantisPending = $returnCountData['total'];
} else {
    // Handle error if the query fails
    // You may want to log or display the error message
}

if ($$RTCWarrantStandingSqlResult) {
    $returnCountData = $$RTCWarrantStandingSqlResult->fetch_assoc();
    $RTCWarrantisStanding = $returnCountData['total'];
} else {
    // Handle error if the query fails
    // You may want to log or display the error message
}
...
$InterpreterInboxPreTrial = 'inbox_interpreter';
$InterpreterInboxSubpoena = 'rtc_submit_subpoena';
$InterpreterPreTrial = 'v5_interpreter_scheduling';
$InterpreterHearing = 'v6_interpreter_hearing';

//Query for Total sent cases
$InterpreterInboxPreTrialQuery = "SELECT COUNT(*) AS total FROM $InterpreterInboxPreTrial";
$InterpreterInboxPreTrialQueryResult = $conn->query($InterpreterInboxPreTrialQuery);

$InterpreterSubpoenaInboxQuery = "SELECT COUNT(*) AS total FROM $InterpreterInboxSubpoena";
$InterpreterSubpoenaInboxQueryResult = $conn->query($InterpreterSubpoenaInboxQuery);

$InterpreterPreTrialQuery = "SELECT COUNT(*) AS total FROM $InterpreterPreTrial";
$InterpreterPreTrialQueryResult = $conn->query($InterpreterPreTrialQuery);

$InterpreterOnGoingQuery = "SELECT COUNT(*) AS total FROM $InterpreterHearing WHERE isFinished = '0'";
$InterpreterOnGoingQueryResult = $conn->query($InterpreterOnGoingQuery);

$InterpreterFinishedQuery = "SELECT COUNT(*) AS total FROM $InterpreterHearing WHERE isFinished = '1'";
$InterpreterFinishedQueryResult = $conn->query($InterpreterFinishedQuery);

// Initialize counts
$InterpreterInboxPreTrialCount = 0;
$InterpreterInboxSubpoenaCount = 0;
$InterpreterPreTrialCount = 0;
$InterpreterHearingisOnGoing = 0;
$InterpreterHearingisFinished = 0;

// Check and set total sent cases count
if ($InterpreterInboxPreTrialQueryResult) {
    $sentCountData = $InterpreterInboxPreTrialQueryResult->fetch_assoc();
    $InterpreterInboxPreTrialCount = $sentCountData['total'];
} else {
    // Handle error if the query fails
    // You may want to log or display the error message
}
...
//Check and set total inbox 
if ($InterpreterSubpoenaInboxQueryResult) {
    $returnCountData = $InterpreterSubpoenaInboxQueryResult->fetch_assoc();
    $InterpreterInboxSubpoenaCount = $returnCountData['total'];
} else {
    // Handle error if the query fails
    // You may want to log or display the error message
}

if ($InterpreterPreTrialQueryResult) {
    $returnCountData = $InterpreterPreTrialQueryResult->fetch_assoc();
    $InterpreterPreTrialCount = $returnCountData['total'];
} else {
    // Handle error if the query fails
    // You may want to log or display the error message
}

if ($InterpreterOnGoingQueryResult) {
    $returnCountData = $InterpreterOnGoingQueryResult->fetch_assoc();
    $InterpreterHearingisOnGoing = $returnCountData['total'];
} else {
    // Handle error if the query fails
    // You may want to log or display the error message
}

if ($InterpreterFinishedQueryResult) {
    $returnCountData = $InterpreterFinishedQueryResult->fetch_assoc();
    $InterpreterHearingisFinished = $returnCountData['total'];
} else {
    // Handle error if the query fails
    // You may want to log or display the error message
}
...
<body class="bg-gray-200">
    ...
    <div>
            ...
            <nav class="fixed top-0 left-0 bottom-0 flex flex-col w-3/4 lg:w-80 sm:max-w-xs pt-6 pb-8 bg-white overflow-y-auto border-l border-gray-400 shadow-2xl">
                ...
                <div class="px-4 pb-6">
                    ...
                    <ul class="text-sm font-medium">
                        ...
                        <li class="<?php echo ($SubmitCase === 0) ? 'hidden' : ''; ?>">
                            <a class="flex items-center pl-3 py-3 pr-2 text-gray-500 hover:bg-gray-700 hover:text-white transition duration-300 border-gray-500 rounded" href="userRoute.php">
                                <span class="inline-block mr-3">
                                    <svg class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M21 12L3 22l4-10-4-10 18 10z">
                                    </path>
                        ...
                        <li class="<?php echo ($Documents === 0) ? 'hidden' : ''; ?>">
                            <a class="flex items-center pl-3 py-3 pr-2 text-gray-500 hover:bg-gray-700 hover:text-white transition duration-300 border-gray-500 rounded" href="userNewDocuments.php">
                                <span class="inline-block mr-3">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
        ...
        <!-- FISCAL DASHBOARD -->
        <div  <div class="<?php echo ($loggedInRole === "Fiscal") ? '' : 'hidden'; ?> mx-auto">>
            <main class="max-w-screen-6xl px-4 mx-auto max-h-screen-xl lg:px-12 md:ml-64 bg-gray-200 min-h-screen transition-all main">
                <div class="max-w-screen-2xl py-2 px-6  flex items-center justify-center  shadow-black/5 sticky top-0">
                    <h1 class="font-semibold font-medium text-2xl">YOUR DASHBOARD</h1>
                </div>

                <div class="p-6 flex items-center just">
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">

                        <div class="bg-white rounded-md border border-gray-100 p-6 px-6 shadow-md shadow-slate-500  transition-all">
                            <div class="flex justify-between mb-6">
                                <div class="text-center">
                                    <h4 class="inline-block mr-2 font-normal"></h4>
                                    <a href="index.php" class="font-medium text-2xl text-black">Inbox</a>
                                    <br>
                                    <h4 class="font-medium text-black text-xl"><?= $FiscalInboxcount; ?></h4>
                                </div>
                            </div>
                            <!-- Button inside the grid -->
                            <div class="flex justify-center">
                                <button class="bg-gray-800 hover:bg-gray-700 text-white font-normal py-2 px-8 rounded" href="userSentCases.php">View Inbox</button>
                            </div>
                        </div>

                        <div class="bg-white rounded-md border border-gray-100 p-6 shadow-md  shadow-slate-500  transition-all">
                            <div class="flex justify-between mb-6">
                                <div class="text-center">
                                    <h4 class="inline-block mr-2 font-normal" style="font-family: 'Poppins';"></h4>
                                    <a href="index.php" class="font-medium text-2xl text-black ">Sent Cases</a>
                                    <br>
                                    <h4 class="font-medium text-black text-xl "><?= $FiscalSentrowCount;?></h4>
                                </div>
                            </div>
                            <!-- Button inside the grid -->
                            <div class="flex justify-center">
                                <button class="bg-gray-800 hover:bg-gray-700 text-white font-normal py-2 px-8 rounded">View Sent Cases</button>
                            </div>
                        </div>
                        ...
                        <div class="bg-white rounded-md border border-gray-100 p-6 shadow-md  shadow-slate-500  transition-all">
                            <div class="flex justify-between mb-6">
                                <div class="text-center">
                                    <h4 class="inline-block mr-2 font-normal" style="font-family: 'Poppins';"></h4>
                                    <a href="index.php" class="font-medium text-2xl text-black ">Returns</a>
                                    <br>
                                    <h4 class="font-medium text-black text-xl "><?= $FiscalReturncount;?></h4>
                                </div>
                            </div>
                            <!-- Button inside the grid -->
                            <div class="flex justify-center">
                                <button class="bg-gray-800 hover:bg-gray-700 text-white font-normal py-2 px-8 rounded">View Returns</button>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <!--PAO Dashboard-->
        <div  <div class="<?php echo ($loggedInRole === "PAO") ? '' : 'hidden'; ?> mx-auto">>
            <main class="max-w-screen-6xl px-4 mx-auto max-h-screen-xl lg:px-12 md:ml-64 bg-gray-200 min-h-screen transition-all main">
                <div class="max-w-screen-2xl py-2 px-6  flex items-center justify-center  shadow-black/5 sticky top-0">
                    <h1 class="font-semibold font-medium text-2xl">YOUR DASHBOARD</h1>
                </div>
                        ...
                        <div class="bg-white rounded-md border border-gray-100 p-6 px-6 shadow-md shadow-slate-500  transition-all">
                            <div class="flex justify-between mb-6">
                                <div class="text-center">
                                    ...
                                    <a href="index.php" class="font-medium text-2xl text-black">Sent Cases</a>
                                    <br>
                                    <h4 class="font-medium text-black text-xl"><?= $PaorowCount; ?></h4>
                                </div>
                            </div>
                            <!-- Button inside the grid -->
                            <div class="flex justify-center">
                                <button class="bg-gray-800 hover:bg-gray-700 text-white font-normal py-2 px-8 rounded" href="userSentCases.php">View Sent Cases</button>
                            </div>
                        </div>
                        ...
                        <div class="bg-white rounded-md border border-gray-100 p-6 shadow-md  shadow-slate-500  transition-all">
                            <div class="flex justify-between mb-6">
                                <div class="text-center">
                                    ...
                                    <a href="index.php" class="font-medium text-2xl text-black ">Returns</a>
                                    <br>
                                    <h4 class="font-medium text-black text-xl "><?= $Returncount;?></h4>
                                </div>
                            </div>
                            <!-- Button inside the grid -->
                            <div class="flex justify-center">
        ...
        <!--OCC DASHBOARD-->
        <div  <div class="<?php echo ($loggedInRole === "OCC") ? '' : 'hidden'; ?> mx-auto">>
            <main class="max-w-screen-6xl px-4 mx-auto max-h-screen-xl lg:px-12 md:ml-64 bg-gray-200 min-h-screen transition-all main">
                <div class="max-w-screen-2xl py-2 px-6  flex items-center justify-center  shadow-black/5 sticky top-0">
                    <h1 class="font-semibold font-medium text-2xl">YOUR DASHBOARD</h1>
                </div>
                        ...
                        <div class="bg-white rounded-md border border-gray-100 p-6 px-6 shadow-md shadow-slate-500  transition-all">
                            <div class="flex justify-between mb-6">
                                <div class="text-center">
                                    ...
                                    <a href="index.php" class="font-medium text-2xl text-black">Inbox</a>
                                    <br>
                                    <h4 class="font-medium text-black text-xl"><?= $OCCInboxcount; ?></h4>
                                </div>
                            </div>
                            <!-- Button inside the grid -->
                            <div class="flex justify-center">
                        ...
                        <div class="bg-white rounded-md border border-gray-100 p-6 shadow-md  shadow-slate-500  transition-all">
                            <div class="flex justify-between mb-6">
                                <div class="text-center">
                                    ...
                                    <a href="index.php" class="font-medium text-2xl text-black ">Sent Cases</a>
                                    <br>
                                    <h4 class="font-medium text-black text-xl "><?= $OCCSentrowCount;?></h4>
                                </div>
                            </div>
                            <!-- Button inside the grid -->
                            <div class="flex justify-center">
        ...
        <!--RTC DASHBOARD-->
        <div  <div class="<?php echo ($loggedInRole === "RTC") ? '' : 'hidden'; ?> mx-auto">>
            <main class="max-w-screen-6xl px-4 mx-auto max-h-screen-xl lg:px-12 md:ml-64 bg-gray-200 min-h-screen transition-all main">
                <div class="max-w-screen-2xl py-2 px-6  flex items-center justify-center  shadow-black/5 sticky top-0">
                    <h1 class="font-semibold font-medium text-2xl">YOUR DASHBOARD</h1>
                </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
                        ...
                        <div class="bg-white rounded-md border border-gray-100 p-6 px-6 shadow-md shadow-slate-500  transition-all">
                            <div class="flex justify-between mb-6">
                                <div class="text-center">
                                    <h4 class="inline-block mr-2 font-normal"></h4>
                                    <a href="index.php" class="font-medium text-2xl text-black">Inbox For Warrant</a>
                                    <br>
                                    <h4 class="font-medium text-black text-xl"><?= $RTCWarrantInbox; ?></h4>
                                </div>
                            </div>
                            <!-- Button inside the grid -->
                            <div class="flex justify-center">
                        ...
                        <div class="bg-white rounded-md border border-gray-100 p-6 px-6 shadow-md shadow-slate-500  transition-all">
                            <div class="flex justify-between mb-6">
                                <div class="text-center">
                                    <h4 class="inline-block mr-2 font-normal"></h4>
                                    <a href="index.php" class="font-medium text-2xl text-black">Inbox For Subpoena</a>
                                    <br>
                                    <h4 class="font-medium text-black text-xl"><?= $RTCSubpoenaInbox; ?></h4>
                                </div>
                            </div>
                            <!-- Button inside the grid -->
                            <div class="flex justify-center">
                        ...
                        <div class="bg-white rounded-md border border-gray-100 p-6 shadow-md  shadow-slate-500  transition-all">
                            <div class="flex justify-between mb-6">
                                <div class="text-center">
                                    <h4 class="inline-block mr-2 font-normal" style="font-family: 'Poppins';"></h4>
                                    <a href="index.php" class="font-medium text-2xl text-black ">Sent Warrant</a>
                                    <br>
                                    <h4 class="font-medium text-black text-xl "><?= $RTCWarrantCount;?></h4>
                                </div>
                            </div>
                            <!-- Button inside the grid -->
                            <div class="flex justify-center">
                                <button class="bg-gray-800 hover:bg-gray-700 text-white font-normal py-2 px-8 rounded">View Sent Warrant</button>
                            </div>
                        </div>
                        ...
                        <div class="bg-white rounded-md border border-gray-100 p-6 shadow-md  shadow-slate-500  transition-all">
                            <div class="flex justify-between mb-6">
                                <div class="text-center">
                                    <h4 class="inline-block mr-2 font-normal" style="font-family: 'Poppins';"></h4>
                                    <a href="index.php" class="font-medium text-2xl text-black ">Subpoena Issued</a>
                                    <br>
                                    <h4 class="font-medium text-black text-xl "><?= $RTCSubpoenaCount;?></h4>
                                </div>
                            </div>
                            <!-- Button inside the grid -->
                            <div class="flex justify-center">
                                <button class="bg-gray-800 hover:bg-gray-700 text-white font-normal py-2 px-8 rounded">View Subpoena</button>
                            </div>
                        </div>
                        ...
                        <div class="bg-white rounded-md border border-gray-100 p-6 shadow-md  shadow-slate-500  transition-all">
                            <div class="flex justify-between mb-6">
                                <div class="text-center">
                                    <h4 class="inline-block mr-2 font-normal" style="font-family: 'Poppins';"></h4>
                                    <a href="index.php" class="font-medium text-2xl text-black ">Pending Warrant</a>
                                    <br>
                                    <h4 class="font-medium text-black text-xl "><?= $RTCWarrantisPending;?></h4>
                                </div>
                            </div>
                            <!-- Button inside the grid -->
                            <div class="flex justify-center">
                                <button class="bg-gray-800 hover:bg-gray-700 text-white font-normal py-2 px-8 rounded">View Warrant</button>
                            </div>
                        </div>
                        ...
                        <div class="bg-white rounded-md border border-gray-100 p-6 shadow-md  shadow-slate-500  transition-all">
                            <div class="flex justify-between mb-6">
                                <div class="text-center">
                                    <h4 class="inline-block mr-2 font-normal" style="font-family: 'Poppins';"></h4>
                                    <a href="index.php" class="font-medium text-2xl text-black ">Standing Warrant</a>
                                    <br>
                                    <h4 class="font-medium text-black text-xl "><?= $RTCWarrantisStanding;?></h4>
                                </div>
                            </div>
                            <!-- Button inside the grid -->
                            <div class="flex justify-center">
        ...
        <div  <div class="<?php echo ($loggedInRole === "Interpreter") ? '' : 'hidden'; ?> mx-auto">>
            <main class="max-w-screen-6xl px-4 mx-auto max-h-screen-xl lg:px-12 md:ml-64 bg-gray-200 min-h-screen transition-all main">
                <div class="max-w-screen-2xl py-2 px-6  flex items-center justify-center  shadow-black/5 sticky top-0">
                    <h1 class="font-semibold font-medium text-2xl">YOUR DASHBOARD</h1>
                </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
                        ...
                        <div class="bg-white rounded-md border border-gray-100 p-6 px-6 shadow-md shadow-slate-500  transition-all">
                            <div class="flex justify-between mb-6">
                                <div class="text-center">
                                    <h4 class="inline-block mr-2 font-normal"></h4>
                                    <a href="index.php" class="font-medium text-2xl text-black">For PreTrial</a>
                                    <br>
                                    <h4 class="font-medium text-black text-xl"><?= $InterpreterInboxPreTrialCount; ?></h4>
                                </div>
                            </div>
                            <!-- Button inside the grid -->
                            <div class="flex justify-center">
                        ...
                        <div class="bg-white rounded-md border border-gray-100 p-6 px-6 shadow-md shadow-slate-500  transition-all">
                            <div class="flex justify-between mb-6">
                                <div class="text-center">
                                    <h4 class="inline-block mr-2 font-normal"></h4>
                                    <a href="index.php" class="font-medium text-2xl text-black">For Hearing</a>
                                    <br>
                                    <h4 class="font-medium text-black text-xl"><?= $InterpreterInboxSubpoenaCount; ?></h4>
                                </div>
                            </div>
                            <!-- Button inside the grid -->
                            <div class="flex justify-center">
                        ...
                        <div class="bg-white rounded-md border border-gray-100 p-6 shadow-md  shadow-slate-500  transition-all">
                            <div class="flex justify-between mb-6">
                                <div class="text-center">
                                    <h4 class="inline-block mr-2 font-normal" style="font-family: 'Poppins';"></h4>
                                    <a href="index.php" class="font-medium text-2xl text-black ">Scheduled Pre Trials</a>
                                    <br>
                                    <h4 class="font-medium text-black text-xl "><?= $InterpreterPreTrialCount;?></h4>
                                </div>
                            </div>
                            <!-- Button inside the grid -->
                            <div class="flex justify-center">
                                <button class="bg-gray-800 hover:bg-gray-700 text-white font-normal py-2 px-8 rounded">View</button>
                            </div>
                        </div>
                        ...
                        <div class="bg-white rounded-md border border-gray-100 p-6 shadow-md  shadow-slate-500  transition-all">
                            <div class="flex justify-between mb-6">
                                <div class="text-center">
                                    <h4 class="inline-block mr-2 font-normal" style="font-family: 'Poppins';"></h4>
                                    <a href="index.php" class="font-medium text-2xl text-black ">On Going Hearing</a>
                                    <br>
                                    <h4 class="font-medium text-black text-xl "><?= $InterpreterHearingisOnGoing;?></h4>
                                </div>
                            </div>
                            <!-- Button inside the grid -->
                            <div class="flex justify-center">
                        ...
                        <div class="bg-white rounded-md border border-gray-100 p-6 shadow-md  shadow-slate-500  transition-all">
                            <div class="flex justify-between mb-6">
                                <div class="text-center">
                                    <h4 class="inline-block mr-2 font-normal" style="font-family: 'Poppins';"></h4>
                                    <a href="index.php" class="font-medium text-2xl text-black ">Hearing Finished</a>
                                    <br>
                                    <h4 class="font-medium text-black text-xl "><?= $InterpreterHearingisFinished;?></h4>
                                </div>
                            </div>
                            <!-- Button inside the grid -->
                            <div class="flex justify-center">
