...
$PaoSentcaseSql = "SELECT * from v1_pao_submit_case";
$PaoSentcaseSqlResult = $conn->query($PaoSentcaseSql);

$inboxSql = "SELECT * FROM inbox_fiscal WHERE Submitted = '0'";
$inboxResult = $conn->query($inboxSql);

$RTCForSubpoenaSql = "SELECT * FROM rtc_for_subpoena WHERE ToEmployee = '$loggedInEmployeeId' AND Submitted = 0";
$RTCForSubpoenaSqlResult = $conn->query($RTCForSubpoenaSql);

$HearingSchedulesSql = "SELECT * FROM v6_interpreter_hearing WHERE isFinished = '1'";
$HearingSchedulesSqlResult = $conn->query($HearingSchedulesSql);

$RouteSql = "SELECT * FROM test_route";
$RouteSqlResult = $conn->query($RouteSql);
...
<body class="bg-gray-200 overflow-y-hidden">
            ...
            <div class="px-4 pb-6">
                ...
                <ul class="text-sm font-medium">
                    ...
                    <li class="<?php echo ($SubmitCase === 0) ? 'hidden' : ''; ?>">
                        <a class="flex items-center pl-3 py-3 pr-2 text-gray-50 bg-gray-800 rounded">
                            <span class="inline-block mr-3">
                    ...
                    <li class="<?php echo ($CaseAssignment === 0) ? 'hidden' : ''; ?>">
                        <a class="flex items-center pl-3 py-3 pr-4 text-gray-50 hover:bg-gray-900 rounded" href="userCaseAssignment.php">
                            <span class="inline-block mr-3">
    ...
    <!--PAO Submit Case Form-->
    <!-- MAIN -->
    
    <form class="my-8 text-sm" action="../../controller/user_submitCaseController.php" method="post" enctype="multipart/form-data">
        <div class="flex max-w-screen-6xl px-4 mx-auto lg:px-12 md:ml-64 bg-gray-200 transition-all main overflow-hidden ml-96">
            <div class="<?php echo ($loggedInRole === "PAO") ? 'flex' : 'hidden'; ?> w-full min-h-screen">
                <!-- Form Section -->
                <div class="w-1/2 bg-gray-200"> <!-- Added bg-gray-200 here -->

                    <div class="lg:flex">
                        <div class="w-full md:w-3/4 lg:w-2/3 xl:w-[500px] 2xl:w-[550px] mt-8 mx-auto px-16 py-8 rounded-lg">
                            <h1 class="font-bold font-serif text-2xl text-[#07074D]">Submit Case</h1>

                            <input type="hidden" name="empId" id="empId" value=<?php echo $loggedInEmployeeId; ?>>
                            <!-- Complainant Name -->
                            <div class="flex flex-col my-4">
                                <label for="complainantName" class="text-gray-700 font-medium">Complainant Name</label>
                                <input required type="text" name="cLName" id="complainantName" class="mt-2 p-2 border border-gray-300 rounded text-sm text-gray-900 focus:border-gray-500 focus:outline-none focus:ring-0" placeholder="Last Name">
                                <input required type="text" name="cFName" id="complainantName" class="mt-2 p-2 border border-gray-300 rounded text-sm text-gray-900 focus:border-gray-500 focus:outline-none focus:ring-0" placeholder="First Name">
                                <input required type="text" name="cMName" id="complainantName" class="mt-2 p-2 border border-gray-300 rounded text-sm text-gray-900 focus:border-gray-500 focus:outline-none focus:ring-0" placeholder="Middle Name">
                            </div>
                            <!-- Accused Name -->
                            <div class="flex flex-col my-4">
                                <label for="accusedName" class="text-gray-700 font-medium">Accused Name</label>
                                <input required type="text" name="aLName" id="accusedName" class="mt-2 p-2 border border-gray-300 rounded text-sm text-gray-900 focus:border-gray-500 focus:outline-none focus:ring-0" placeholder="Last Name">
                                <input required type="text" name="aFName" id="accusedName" class="mt-2 p-2 border border-gray-300 rounded text-sm text-gray-900 focus:border-gray-500 focus:outline-none focus:ring-0" placeholder="First Name">
                                <input required type="text" name="aMName" id="accusedName" class="mt-2 p-2 border border-gray-300 rounded text-sm text-gray-900 focus:border-gray-500 focus:outline-none focus:ring-0" placeholder="Middle Name">
                            </div>
                            <select name="Route" id="Route" class="mt-2 p-2 border border-gray-300 focus:outline-none focus:ring-0 focus:border-gray-300 rounded text-sm text-gray-900">
                                <?php foreach ($RouteSqlResult as $Route) : ?>
                ...
                <!-- File Upload Section -->
                <div class="hidden lg:flex lg:flex-col lg:w-1/2 lg:pl-8 mb-40">
                    <!--upload file-->
                    <div class="mb-8 mt-24 rounded-lg border border-[#e0e0e0] p-2 text-center shadow-md bg-white">
                        <label for="file" class="relative flex min-h-[200px] items-center justify-center">
                            <div>
                                <div class="flex items-center justify-center">
                                    <svg width="84" height="84" viewBox="0 0 96 96" xmlns="http://www.w3.org/2000/svg">
                                        <g clip-path="url(#clip0_485_117)">
                                            <path d="M77.4 40.16C74.68 26.36 62.56 16 48 16C36.44 16 26.4 22.56 21.4 32.16C9.36 33.44 0 43.64 0 56C0 69.24 10.76 80 24 80H76C87.04 80 96 71.04 96 60C96 49.44 87.8 40.88 77.4 40.16ZM56 52V68H40V52H28L48 32L68 52H56Z" fill="gray-800" />
                                        </g>
                                        <defs>
                                ...
                                <span class="mb-2 block text-xl md:text-xl font-semibold text-gray-800">
                                    Drag & Drop files or <span class="font-medium underline underline-blue">browse</span>
                                </span>
                                <span class="py-1 px-5 md:py-2 md:px-7 font-medium text-gray-400">
                                    Supported Formats: doc, docx
                                </span>
                            </div>
                        </label>
                        <input class="bg-white ml-12" required type="file" name="file" accept=".doc, .docx" id="file" onchange="handleFileSelect(event)" />
                    </div>
                    <!--upload file-->
                    <!-- <div class="flex flex-col my-4">
                        <input required type='file' name="file" accept=".doc, .docx" onchange="handleFileSelect(event)" />
                    </div> -->
                    <!-- Submit Button -->
                    <div class="mr-3.5  flex items-center justify-center space-x-4">
                        <button type="submit" name="paoSubmit" id="paoSubmit" class="bg-gray-700 hover:bg-gray-900 font-semibold rounded-2xl w-full px-8 py-2 text-gray-100 hover:shadow-xl transition duration-150 uppercase">Submit</button>
                    </div>
                    <div class="flex items-center justify-center mt-8">
                        <img src="../../images/submitcase.gif" class="max-w-xs object-contain"> <!-- Adjust size as needed -->
                    </div>
                </div>
            </div>
        </div>
    ...
    <!--Fiscal Button for private lawyer-->
    <div class="flex max-w-screen-6xl px-4 mx-auto lg:px-12 md:ml-64 bg-gray-200 transition-all main overflow-hidden ml-96">
        <div class="<?php echo ($loggedInRole === "Fiscal") ? 'flex' : 'hidden'; ?> w-full min-h-screen">
            <div class="w-1/2 bg-gray-200">
                <div class="w-full md:w-3/4 lg:w-2/3 xl:w-[500px] 2xl:w-[550px] mb-32 mx-auto px-16 py-8 rounded-lg">
                    <h1 class="font-bold font-serif text-2xl text-[#07074D]">Submit Case</h1>
                    <form class="my-8 text-sm" action="../../controller/user_submitCaseController.php" method="post" enctype="multipart/form-data">
                        <input type="hidden" name="empId" id="empId" value=<?php echo $loggedInEmployeeId; ?>>
                        <!-- Case Title -->
                        <div class="flex flex-col my-4">
                            <label for="caseTitle" class="text-gray-700 font-semibold">Case Title</label>
                            <input required type="text" name="caseTitle" id="caseTitle" class="mt-2 p-2 border border-gray-300 focus:outline-none focus:ring-0 focus:border-gray-300 rounded text-sm text-gray-900">
                        </div>
                        <!-- Penalty -->
                        <div class="flex flex-col my-4">
                        ...
                        <div class="flex flex-col mb-28">
                            <label for="inboxFile" class="text-gray-700 font-semibold">Select a file from inbox</label>
                            <select name="inboxFile" id="inboxFile" class="mt-2 p-2 border border-gray-300 focus:outline-none focus:ring-0 focus:border-gray-300 rounded text-sm text-gray-900">
                                <?php foreach ($inboxResult as $inbox) : ?>
                                    <option value="<?= $inbox['id']; ?>"><?= $inbox['FileName']; ?></option>
                                <?php endforeach; ?>
                            </select>
                            <!-- File Upload Section -->
                            <div class="flex flex-col my-4">
                                <label for="file" class="text-gray-700 font-semibold">Upload File</label>
                                <input required type='file' name="file" id="file" accept=".doc, .docx" onchange="handleFileSelect(event)" class="font-semibold mt-2 p-2 border border-gray-300 focus:outline-none focus:ring-0 focus:border-gray-300 rounded text-sm text-gray-900" />
                            </div>
                            <!-- Submit Button -->
                            <div class="ml-[value] flex items-center justify-center space-x-4">
                                <div>
                                    <button type="submit" name="fiscalSubmit" id="fiscalSubmit" class="bg-gray-700 hover:bg-gray-900 rounded-2xl px-8 py-2 text-gray-100 hover:shadow-xl transition duration-150 uppercase">Submit</button>
                                </div>
                                <!-- <div class="<?php echo ($loggedInRole === "Fiscal") ? '' : ''; ?> ">
                                    <button type="button" id="addPenalty" class="bg-gray-700 hover:bg-gray-900 rounded-2xl px-8 py-2 text-gray-100 hover:shadow-xl transition duration-150 uppercase">Private</button>
                                </div> -->
                            </div>
                        </div>
                </div>
            ...
            <!-- File Upload Section -->
            <div class="w-full">
                <img src="../../images/boypack.gif" alt="Description" class="w-full h-auto object-contain lg:w-120 lg:h-120 xl:w-150 xl:h-150 mx-auto" />
            </div>
            </form>
    ...
    <!--FOR RTC EMPLOYEE For Subpoena-->
    <div class="<?php echo ($loggedInRole === 'RTC') ? '' : 'hidden'; ?>">
        <div class="ml-40 mr-5 pl-44">
            <div class="lg:flex space-x-16 ml-0.5 mr-32">
                <h1 class="font-bold text-2xl font-serif mb-20">Subpoena Submission</h1>
            </div>
            <div class="mx-auto py-8 w-full mb-40">
                <div class="flex border-gray-500 bg-white rounded-lg shadow-2xl mx-auto px-12 py-8 max-w-6xl">
                    <form class="text-sm w-1/2" action="../../controller/user_submitCaseController.php" method="post" enctype="multipart/form-data">
                        <input type="hidden" name="empId" id="empId" value="<?php echo $loggedInEmployeeId; ?>">
                        <!-- Docket Number-->
                        <div class="border bg-gray-500 rounded-lg px-2 py-2 mr-20 mt-10">
                            <div class="flex flex-col my-4">
                                <label for="docketNumber" class="text-gray-100 font-semibold font-2xl">Docket Number</label>
                                <select name="docketNumber" id="docketNumber" class="mt-2 p-2 border border-gray-300 focus:outline-none focus:ring-0 focus:border-gray-300 rounded text-sm text-gray-900">
                                    <?php foreach ($RTCForSubpoenaSqlResult as $RTCSubpoena) : ?>
                                        <option value="<?= $RTCSubpoena['id']; ?>"><?= $RTCSubpoena['DocketNumber']; ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                            <div class="flex flex-col my-4">
                                <label for="warrantFileName" class="text-gray-100">Choose Subpoena from Desktop:</label>
                            </div>
                            <!-- Upload File -->
                            <div class="flex flex-col my-4">
                                <label for="file" class="text-gray-100 font-semibold">Upload File</label>
                                <input required type="file" name="file" accept=".doc, .docx" onchange="handleFileSelect(event)" class="mt-2 p-2 border border-gray-300 focus:outline-none focus:ring-0 focus:border-gray-300 rounded text-sm text-gray-100 font-semibold">
                            </div>
                            <!-- Submit Button -->
                            <div class="my-4 flex items-center justify-center space-x-4">
                                <button type="submit" name="subpoenaSubmit" id="subpoenaSubmit" class="bg-gray-700 hover:bg-gray-900 rounded-2xl w-full px-8 py-2 text-gray-100 hover:shadow-xl transition duration-150 uppercase font-semibold">Submit</button>
                            </div>
                        </div>
                    </form>
                    <div class="w-1/2 flex justify-center items-center">
                        <img src="../../images/stand.gif" alt="Description" class="w-60 h-60 lg:w-96 lg:h-96 xl:w-120 xl:h-120 object-contain mx-auto lg:mx-0 ml-20 mb-4">
                    </div>
                </div>
            </div>
        </div>
    ...
    <!--SUBMIT CASE FOR INTERPRETER-->

    <!--SUBMIT CASE FOR INTERPRETER-->
            <div class="flex max-w-screen-6xl px-4 mx-auto max-h-screen-xl lg:px-12 md:ml-64 bg-gray-200 min-h-screen transition-all main">
                <div class="<?php echo ($loggedInRole === "Interpreter") ? '' : 'hidden'; ?> w-full min-h-screen flex flex-col">
                    <!-- Form Section -->
                    <div class="w-full lg:w-1/2 bg-gray-200">
                        <div class="lg:flex flex-wrap">
                            <div class="w-full lg:w-2/3 xl:w-[500px] 2xl:w-[550px] mt-8 mx-auto lg:ml-0 lg:mr-72 px-8 py-8 rounded-lg"> 
                                <h1 class="font-bold text-2xl text-[#07074D] lg:text-left">Hearing Summary Submission</h1>
                                <form class="my-8 text-sm" action="../../controller/user_submitCaseController.php" method="post" enctype="multipart/form-data">
                                    <input type="hidden" name="empId" id="empId" value="<?php echo $loggedInEmployeeId; ?>">
                                    
                                    <div class="flex flex-col my-4">
                                        <label for="docketNumber" class="text-gray-700">Docket Number</label>
                                        <select name="docketNumber" id="docketNumber" class="mt-2 p-2 border border-gray-300 focus:outline-none focus:ring-0 focus:border-gray-300 rounded text-sm text-gray-900">
                                            <?php foreach ($HearingSchedulesSqlResult as $HearingSummary) : ?>
                                                <option value="<?= $HearingSummary['id']; ?>"><?= $HearingSummary['DocketNumber']; ?></option>
                                            <?php endforeach; ?>
                                        </select>
                                    </div>
                                    <!-- <div class="flex flex-col my-4">
                                        <label for="FileName" class="text-gray-700">File Name</label>
                                        <select name="FileName" id="FileName" class="mt-2 p-2 border border-gray-300 focus:outline-none focus:ring-0 focus:border-gray-300 rounded text-sm text-gray-900">
                                            <?php foreach ($HearingSchedulesSqlResult as $HearingSummary) : ?>
                                                <option value="<?= $HearingSummary['id']; ?>"><?= $HearingSummary['FileName']; ?></option>
                                            <?php endforeach; ?>
                                        </select>
                                    </div> -->
                                    <div class="flex flex-col my-4">
                                        <label for="warrantFileName" class="text-gray-700">Choose from Desktop:</label>
                                    </div>
                                    ...
                                    <!-- Submit Button -->
                                    <div class="my-4 flex items-center justify-center space-x-4">
                                        <button type="submit" name="summarySubmit" id="summarySubmit" class="bg-gray-900 hover:bg-indigo-700 rounded-lg px-8 py-2 text-gray-100 hover:shadow-xl transition duration-150 uppercase">Submit</button>
                                    </div>
                                </form>
                            </div>
                        </div>
