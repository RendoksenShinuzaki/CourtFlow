...
<body class = "bg-gray-200">
                ...
                <ul class="text-sm font-medium">
                    ...
                    <li class="<?php echo ($SubmitWarrant === 0) ? 'hidden' : ''; ?>">
                        <a class="flex items-center pl-3 py-3 pr-2 text-gray-500 hover:bg-gray-700 hover:text-white transition duration-300 border-gray-500 rounded" href="userSubmitWarrant.php">
                            <span class="inline-block mr-3">
                                <svg class="text-gray-600 w-5 h-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
    ...
    <?php
        if ($loggedInRole === "PAO") {
        $id = $_GET["id"];
        $PaoQuery = "SELECT * FROM v1_pao_submit_case WHERE id = ?";
    
        // Use prepared statement
        $statement = $conn->prepare($PaoQuery);
        $statement->bind_param("i", $id);
        $statement->execute();
        $PaoQueryResult = $statement->get_result();
    
        if ($PaoQueryResult->num_rows > 0) {
            $PAOrow = $PaoQueryResult->fetch_assoc();
    
            // Define the mapping of step numbers to department names
            $stepToDepartment = [
                1 => 'Fiscal Department',
                2 => 'OCC Department',
                3 => 'RTC Department',
                4 => 'Interpreter Department',
                5 => 'Scheduled Pre Trial',
                6 => 'Scheduled Hearing',
                7 => 'File indexed'
            ];
            ...
            // Progress Bar
            $totalSteps = 7;
            $currentProgress = $PAOrow['Version'];
    
                // Back Button
                echo '  <div class="text-center">';
                echo '  <div class="text-left sm:ml-64 sm:mt-12">';
                echo '    <a href="../user/userQRScanner.php" class="inline-flex items-center mb-8 bg-gray-200 hover:bg-gray-300 text-gray-800 font-bold py-2 px-4 rounded">';
                echo '      <svg class="mr-2 w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke        -width              ="2" d="M15 19l-7-7 7-7"></path></svg>';
                echo '      Back';
                echo '    </a>';
                echo ' </div>';
                
                // Start of the progress-container div
                echo '<div class="ml-64 py-4 mr-52 pl-2 border-gray-500 rounded text-center bg-white shadow-2xl">';
                echo '<ol class="mx-auto w-full space-y-4 sm:flex sm:space-x-8 sm:space-y-0 rtl:space-x-reverse">';
                ...
                echo '<li class="flex items-center text-' . $colorClass . '-700 dark:text-' . $colorClass . '-500 space-x-2.5 rtl:space-x-reverse">';
                echo '<span class="flex items-center justify-center w-8 h-8 border border-' . $colorClass . '-700 rounded-full shrink-0 dark:border-' . $colorClass . '-700">';
                echo $step;
                echo '</span>';
                echo '<span>';
                echo '<h3 class="font-medium leading-tight">' . $stepToDepartment[$step] . '</h3>';
                echo '<p class="text-sm">' . $statusText . '</p>';
                echo '</span>';
                echo '</li>';
            }
                echo '</ol>'; // End of the ordered list
                echo '</div>'; // End of the progress-container div
                
                //Border after progress Bar
                echo '<div class="ml-64 py-2 mr-52 pl-2 pb-24 border-gray-500 rounded text-center mt-8 bg-white shadow-2xl">';
                echo '<div class="max-w-md h-96 p-4 inline-block">';
                
                echo '<div class="border-b">'; // Container with a bottom border
                
                // Complainant Name
                echo '<div class="text-left">Complainant Name: ' . $PAOrow["complainantLName"] . ' ' . $PAOrow["complainantFName"] . ' ' . $PAOrow["complainantMName"] . '</div>';
                echo '<div class="text-left">Accused Name: ' . $PAOrow["accusedLName"] . ' ' . $PAOrow["accusedFName"] . ' ' . $PAOrow["accusedMName"] . '</div>';
                echo '<div class="text-left">File Name: ' . $PAOrow['FileName'] . '</div>';
                
                // Stepper
                echo '<div class="border-t border-x px-2 py-2 mt-4 mb-4 ml-2 mr-2 rounded-2xl bg-gray-200 shadow-2xl">'; // Added margins
                echo '<ol class="text-gray-700 dark:text-gray-600">';
                
                $TrackingQuery = "SELECT * FROM tracking WHERE V1_CaseId = '$id' ORDER BY id DESC";
                $TrackingResult = $conn->query($TrackingQuery);
                
                foreach ($TrackingResult as $tracking) {
                echo '<li class="mb-2">';
                echo '<div class="grid grid-cols-2 gap-4">'; // Two-column grid layout
            
                // First column for date, time, AM/PM (right-aligned)
                echo '<div class="text-right col-span-1">';
                echo '<p class="text-base text-black">' . htmlspecialchars($tracking["Date"]) . ' ' . $tracking["Time"] . ' ' . $tracking["AmPm"] . '</p>';
                echo '</div>';
            
                // Second column for the message (left-aligned)
                echo '<div class="text-left col-span-1">';
                echo '<p class="text-base text-gray-500">' . $tracking["Message"] . '</p>';
                echo '</div>';
                ...
                echo '</div>'; // End of the max-w-md h-96 p-2 inline-block div
                echo '</div>'; // End of the ml-4 py-2 mr-4 pl-1 pb-8 border-gray-500 rounded text-center mt-2 bg-white shadow-2xl div
               ...
               //Back Button
                echo '<div class="text-center">';
                echo '<div class="text-left sm:ml-64 sm:mt-12">';
                echo '<a href="../user/userQRScanner.php" class="inline-flex items-center mb-8 bg-gray-200 hover:bg-gray-300 text-gray-800 font-bold py-2 px-4 rounded text-lg">';
                echo '<svg class="mr-2 w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke            -width ="2" d="M15 19l-7-7 7-7"></path></svg>';
                echo 'Back';
                echo '</a>';
                echo '</div>';
                //Wrong File
                echo '<div class="flex justify-center items-center mt-16">';
                echo '<div class="text-center">';
                echo '<div class="mb-4 text-5xl font-semibold text-gray-600">No results found</div>';
                echo '<img src="../../images/WrongFile.gif" class="mx-auto w-full max-w-7xl" alt="No records">';
                echo '</div>';
                echo '</div>';
            }
        }
    ...
    <?php
    if ($loggedInRole === "Fiscal") {
        $id = $_GET["id"];
        $FiscalQuery = "SELECT * FROM v2_fiscal_submit_case WHERE id = ?";
    
        // Use prepared statement
        $statement = $conn->prepare($FiscalQuery);
        $statement->bind_param("i", $id);
        $statement->execute();
        $FiscalQueryResult = $statement->get_result();

        if ($FiscalQueryResult->num_rows > 0) {
            $Fiscalrow = $FiscalQueryResult->fetch_assoc();
            ...
            // Define the mapping of step numbers to department names
                $stepToDepartment = [
                    ...
                    5 => 'Scheduled Pre Trial',
                    6 => 'Scheduled Hearing',
                    7 => 'File being indexed'
                ];
        
                // Progress Bar
                $totalSteps = 7;
                $currentProgress = $Fiscalrow['Version'];
               ...
               for ($step = 1; $step <= $totalSteps; $step++) {
                ...
                $V1_CaseId = $Fiscalrow["V1_CaseId"];
                $PaoQuery = "SELECT * FROM v1_pao_submit_case WHERE id = ?";
    
                // Use prepared statement
                $statement = $conn->prepare($PaoQuery);
                $statement->bind_param("i", $V1_CaseId);
                $statement->execute();
                $PaoQueryResult = $statement->get_result();
                $PAOrow = $PaoQueryResult->fetch_assoc();
                ...
                echo '<div class = "text-left">Complainant Name: ' . $PAOrow["complainantLName"] . ' ' . $PAOrow["complainantFName"] . ' ' . $PAOrow["complainantMName"] . '</div>';
                echo '<div class = "text-left">Accused Name: ' . $PAOrow["accusedLName"] . ' ' . $PAOrow["accusedFName"] . ' ' . $PAOrow["accusedMName"] . '</div>';
                echo '<div class = "text-left">Penalty: ' . $Fiscalrow['Penalty'] . '</div>';
                echo '<div class = "text-left">File Name: ' . $Fiscalrow['FileName'] . '</div>';
                
                //stepper 
                echo '<div class="border-t border-x px-2 py-2 rounded-2xl bg-gray-200 shadow-2xl">';  // Stepper with top and horizontal borders only
                echo '<ol class="text-gray-700 dark:text-gray-600">';
                
                $TrackingQuery = "SELECT * FROM tracking WHERE V2_CaseId = ?";
                // Use prepared statement
                $statement = $conn->prepare($TrackingQuery);
                $statement->bind_param("i", $id);
                $statement->execute();
                $TrackingQueryResult = $statement->get_result();
                $TrackingRow = $TrackingQueryResult->fetch_assoc();
                
                echo '<li class = "mb-2">';
                echo '<div class="grid grid-cols-2 gap-4">'; // Two-column grid layout
                
                //First Column date to AmPm (Right-align)
                echo '<div class="text-right col-span-1">';
                echo '<p class="text-base text-black">' . htmlspecialchars($TrackingRow["Date"]) .' '. $TrackingRow["Time"].' ' . $TrackingRow["AmPm"] . ' </p>';
                echo '</div>';
                
                //Second Column for the message (Left-align)
                echo '<div class="text-left col-span-1">';
                echo '<p class="text-base text-gray-500">' . $TrackingRow["Message"] . '</p>';
                echo '<div>';
                ...
                echo '</div>'; // End of the max-w-md h-96 p-4 inline-block div
                echo '</div>'; // End of the ml-96 py-4 mr-32 pl-2 border-gray-500 rounded text-center mt-52 bg-white shadow-2xl div
            } else {
                //Back Button
                echo '<div class="text-center">';
                echo '<div class="text-left sm:ml-64 sm:mt-12">';
     ...
     <?php
        if ($loggedInRole === "OCC") {
        $id = $_GET["id"];
        $OCCQuery = "SELECT * FROM v3_occ_case_assignment WHERE id = ?";
    
        // Use prepared statement
        $statement = $conn->prepare($OCCQuery);
        $statement->bind_param("i", $id);
        $statement->execute();
    
        $OCCQueryResult = $statement->get_result();
    
        if ($OCCQueryResult->num_rows > 0) {
            $OCCrow = $OCCQueryResult->fetch_assoc();
            ...
            // Progress Bar
            $totalSteps = 7;
            $currentProgress = $OCCrow['Version'];
    
            // Start of Progress Container
            echo '<div class="text-center">';
            echo '<div class="text-left sm:ml-64 sm:mt-12">';
            echo '<a href="../user/userQRScanner.php" class="inline-flex items-center mb-8 bg-gray-200 hover:bg-gray-300 text-gray-800 font-bold py-2 px-4 rounded">';
            echo '<svg class="mr-2 w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path></svg>';
            ...
            for ($step = 1; $step <= $totalSteps; $step++) {
                ...
                $V2_CaseId = $OCCrow["V2_CaseId"];
                $FiscalQuery = "SELECT * FROM v2_fiscal_submit_case WHERE id = ?";
    
                // Use prepared statement
                $statement = $conn->prepare($FiscalQuery);
                $statement->bind_param("i", $V2_CaseId);
                $statement->execute();
                $FiscalQueryResult = $statement->get_result();
                $Fiscalrow = $FiscalQueryResult->fetch_assoc();

                //Complainant Name
                echo '<div class = "text-left">Docket Number: ' . $OCCrow["DocketNumber"] .'</div>';
                echo '<div class = "text-left">Branch Assigned: ' . $OCCrow["ToBranch"] . '</div>';
                echo '<div class = "text-left">Docket File Name: ' . $OCCrow['FileName'] . '</div>';
                echo '<div class = "text-left">Date Submitted: ' . $OCCrow['DateSubmitted'] . '</div>';
                
                //stepper 
                echo '<div class="border-t border-x px-2 py-2 rounded-2xl bg-gray-200 shadow-2xl">';  // Stepper with top and horizontal borders only
                echo '<ol class="text-gray-700 dark:text-gray-600">';
                
                $TrackingQuery = "SELECT * FROM tracking WHERE V3_CaseId = ?";
                // Use prepared statement
                $statement = $conn->prepare($TrackingQuery);
                $statement->bind_param("i", $id);
                $statement->execute();
                ...
                $TrackingQuery = "SELECT * FROM tracking WHERE V3_CaseId = '$id' ORDER BY id DESC";
                $TrackingResult = $conn->query($TrackingQuery);
                ...
                echo '</div>'; // Close grid
                echo '</li>';
            } // End of the ml-96 py-4 mr-32 pl-2 border-gray-500 rounded text-center mt-52 bg-white shadow-2xl div
            } else {
                //Back Button
                echo '<div class="text-center">';
                echo '<div class="text-left sm:ml-64 sm:mt-12">';
