...
$auditTrailSql = "SELECT * FROM audit_trail";
$auditTrailResult = $conn->query($auditTrailSql);

//Pagination Variables
$num_per_page = 5; //Number of records
$total_records = $auditTrailResult->num_rows; // Total number of records
...
$auditTrailSql = "SELECT * FROM audit_trail ORDER BY id DESC LIMIT $offset, $num_per_page";
$auditTrailResult = $conn->query($auditTrailSql);
            ...
            <nav class="fixed top-0 left-0 bottom-0 flex flex-col w-3/4 lg:w-80 sm:max-w-xs pt-6 pb-8 bg-white overflow-y-auto border-l border-gray-400 shadow-2xl">
                ...
                <div class="px-4 pb-6">
                    ...
                    <ul class="text-sm font-medium">
                        ...
                        <li>
                            <a href="admin_casesReturns.php" class="flex items-center pl-3 py-3 pr-2 text-gray-500 hover:bg-gray-700 hover:text-white hover:border border-gray-500 rounded" href="#">
                                <span class="inline-block mr-3">
                        ...
                        <li>
                            <a class="flex items-center pl-3 py-3 pr-2 text-white bg-gray-900 rounded">
                                <span class="inline-block mr-3">
                                    <svg class="text-white w-5 h-5" viewbox="0 0 20 16" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <section class="py-20 ml-40 mr-5 pl-44">
        <table class="mt-5 table-auto w-full border-b border-gray-900 bg-gray-600 shadow-2xl rounded-lg">
            <thead class="border-b-2 border-gray-900">
                <tr>
                    <th class="pl-6 py-6">
                        <p class="flex items-center text-xs text-white">Date</p>
                    </th>
                    <th>
                        <p class="flex items-center text-xs text-white">Time</p>
                    </th>
                    <th>
                        <p class="flex items-center text-xs text-white">Audit Trail Message</p>
                    </th>
                </tr>
            </thead>
            <tbody>
                <?php
                foreach ($auditTrailResult as $auditTrail) :
                ?>
                    <tr class="text-xs bg-blue-50 border-b border-gray-100 edit-trigger">
                        <td class="hidden"><?= $auditTrail['id']; ?></td>
                        <td class="pl-6 py-6 bg-blue-100"><?= $auditTrail['Date']; ?></td>
                        <td class="pl-6 py-6 "><?= $auditTrail['Time'] ." ". $auditTrail['AmPm']; ?></td>
                        <td class="pl-6 py-6 bg-blue-100"><?= $auditTrail['Message']; ?></td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
